...
public class DriveQuickstart {
    ...
    private static final Logger LOGGER = Logger.getLogger(DriveQuickstart.class
			.getName());
	...
	public static void main(String[] args) throws GeneralSecurityException, IOException {
		...
		uploadFile("json.json");
		uploadFile("text2.txt");
*/
	...
	public static void moveFileToFolder(String fileId,String folderId) throws IOException, GeneralSecurityException {
		...
		//2º Actualiza el fichero añadiendo como parent la carpeta destino y borrando la carpeta origen
	...
	public static void uploadFile(String name) throws IOException, GeneralSecurityException {
		File fileMetadata = new File();
		String type=null;
		if(name.contains(".ang")||name.contains(".ctl")||name.contains(".r")||name.contains(".yaml")) {
	...
	public static File getDirectoryByName(String name,String project, String workspace,String owner) throws IOException, GeneralSecurityException {
		File res;
		...
		do {
		  FileList result = DriveQuickstart.driveService().files().list()
			//Busca el fichero dentro del proyecto	  
		      .setQ("'"+file.getId()+"' in parents and mimeType='application/vnd.google-apps.folder' and name='"+name+"'")
		      .setSpaces("drive")
	...
	public static File getFileByName(String name,String project, String workspace,String owner) throws IOException, GeneralSecurityException {
		File res;
		String pageToken = null;
		File file=getProjectByName(project, workspace, owner);
		//Si no existe el proyecto no devuelve nada
	...
	public static File getProjectByName(String name,String workspace,String owner) throws IOException, GeneralSecurityException {
		File res;
		String pageToken = null;
		...
		do {
		  FileList result = DriveQuickstart.driveService().files().list()
			...
			//y que tenga el mismo nombre que el usuario	  
		      .setQ("'"+getRepoFolder(owner).getId()+"' in parents and mimeType='application/vnd.google-apps.folder' and name='"+name+"'")
		      .setSpaces("drive")
		      .setFields("nextPageToken, files(id, name)")
		      .setPageToken(pageToken)
		      .execute();
	...
	private static void deleteFolder() throws IOException, GeneralSecurityException {
		...
		do {
		  FileList result = service.files().list()
	          .setQ("name contains 'New directory'")
		      .setFields("nextPageToken, files(id, name)")
		      .setPageToken(pageToken)
		      .execute();
	...
	private static void download() throws IOException, GeneralSecurityException {
	    final NetHttpTransport HTTP_TRANSPORT = GoogleNetHttpTransport.newTrustedTransport();
        Drive service = new Drive.Builder(HTTP_TRANSPORT, JSON_FACTORY, getCredentials(HTTP_TRANSPORT))
                .setApplicationName(APPLICATION_NAME)
                .build();
        //Solo funciona con los archivos que se haya subido desde la aplicacion
		String fileId = "11ggYteCnMQCdMIk6nY06ayJP5KfmpxxE";
